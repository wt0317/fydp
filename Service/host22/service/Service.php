<?php
	...
	class Service {
		...
		public function clearInventory($username, $password){
			...
			mysql_query("DELETE FROM pendingInventory WHERE username='".$username."'");
			
			$DB_Connect->close();
			return array("Success" => "true");
		}
		
		public function adminClear($username, $password){
			$DB_Connect = new DB_Connect();
			$con = $DB_Connect->connect();
			...
			mysql_query("TRUNCATE TABLE inventory");
			mysql_query("TRUNCATE TABLE pendingInventory");
